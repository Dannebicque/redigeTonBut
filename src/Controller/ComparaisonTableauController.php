...
#[Route('/comparaison/tableau', name: 'comparaison_tableau_')]
class ComparaisonTableauController extends BaseController
{
    ...
    public function structure(ApcParcoursRepository $apcParcoursRepository): Response
    {
        ...
        return $this->render('comparaison/tableau/structure.html.twig', [
            'parcours' => $parcours
        ]);
    }

    public function tableauStructureParcours(
        Structure $structure,
        ...
        if ($parcours instanceof ApcParcours && $this->getDepartement()->getTypeStructure() === Departement::TYPE3) {
            ...
            if ($this->getVersion()->getPreviousVersion() !== null) {
                $semestresPrev = $semestreRepository->findByVersion($this->getVersion()->getPreviousVersion());
            } else {
                $semestresPrev = [];
            }
        }

        $jsonCurrent = $structure->setSemestres($semestres)->setVersion($this->getVersion())->getDataJson();

        if (count($semestresPrev) > 0) {
            $jsonPrevious = $structure->setSemestres($semestresPrev)->setVersion($this->getVersion()->getPreviousVersion())->getDataJson();
        } else {
            $jsonPrevious = [];
        }

        return $this->render('comparaison/tableau/_structure.html.twig', [
            'parcours' => $parcours,
            'semestres' => $semestres,
